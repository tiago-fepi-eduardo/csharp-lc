...
namespace Lc.Csharp.Modulo2.Linq
{
    class LinqPratica
    {
        ...
        //    /*
        //    Quantos alunos são imigrantes:
        //    Quantos alunos possuem media maior que 9:
        //    Quantos alunos possuem 12 anos e media maior que 8:
        ...
        //    var r1 = alunos.Where(c => c.Imigrante == true);
        //    Console.WriteLine($"Quantos alunos são imigrantes: {r1.Count()}");

        //    var r2 = alunos.Where(c => c.Media > 9);
        //    Console.WriteLine($"Quantos alunos possuem media maior que 9: {r2.Count()}");

        //    var r3 = alunos.Where(c => c.Media > 8 && c.Idade == 12);
        //    Console.WriteLine($"Quantos alunos possuem 12 anos e media maior que 8: {r3.Count()}");

        //    Console.WriteLine("\nListe alunos por ordem alfabética");
        //    var r4 = alunos.OrderBy(c => c.Nome);
        //    foreach (var item in r4)
        //        Console.Write($"  {item.Nome}");

        //    Console.WriteLine("\nListe os alunos por ordem decrescente de idade e após por alfabética de nome");
        //    var r5 = alunos.OrderByDescending(c => c.Idade).ThenBy(c => c.Nome);
        //    foreach (var item in r5)
        //        Console.Write($"  {item.Nome}");

        //    Console.WriteLine("\nListe alunos por ordem crescent de media, depois por ordem alfabetica de nome e que não são imigrantes");
        //    var r6 = alunos.Where(c => c.Imigrante == false).OrderBy(c => c.Media).ThenBy(c => c.Nome);
